...
public class StorageBridge {
    ...
    public String status() {
        Uri tree = getTreeUri();
        return tree == null ? "NEEDS_PERMISSION" : "OK|" + tree.toString();
    }

    @JavascriptInterface
    public void requestRoot() {
        activity.runOnUiThread(new Runnable() {
            @Override public void run() { activity.launchStorageTree(); }
        });
    }
    ...
    @JavascriptInterface
    public boolean ensureBaseFolders() {
        try {
            Uri root = appRoot(true);
            if (root == null) return false;
            ensureDir(root, "Cierres");
            ensureDir(root, "Reportes");
            ensureDir(root, "Tickets");
            ensureDir(root, "Datos");
            ensureDir(root, "Respaldos");
            return true;
        } catch (Throwable t) { return false; }
    }

    @JavascriptInterface
    public String beginWrite(String relativePath, String mime) {
        if (getTreeUri() == null) return "ERR:NO_PERMISSION";
        String token = UUID.randomUUID().toString();
        synchronized (buffers) {
            buffers.put(token, new ByteArrayOutputStream());
            paths.put(token, normalize(relativePath));
            mimes.put(token, mime == null || mime.trim().isEmpty() ? "application/octet-stream" : mime);
        }
        return token;
    }

    @JavascriptInterface
    public String appendWrite(String token, String base64) {
        try {
            ByteArrayOutputStream out;
            synchronized (buffers) { out = buffers.get(token); }
            if (out == null) return "ERR:TOKEN";
    ...
    @JavascriptInterface
    public String finishWrite(String token) {
        ByteArrayOutputStream out;
        String path, mime;
        synchronized (buffers) {
            out = buffers.remove(token);
            path = paths.remove(token);
            mime = mimes.remove(token);
        }
        if (out == null || path == null) return "ERR:TOKEN";
        try {
            Uri uri = createFilePath(path, mime, true);
            if (uri == null) return "ERR:CREATE";
            OutputStream os = activity.getContentResolver().openOutputStream(uri, "w");
            if (os == null) return "ERR:OPEN";
    ...
    @JavascriptInterface
    public String writeBase64(String relativePath, String mime, String base64) {
        try {
            Uri uri = createFilePath(normalize(relativePath), mime, true);
            if (uri == null) return "ERR:CREATE";
            OutputStream os = activity.getContentResolver().openOutputStream(uri, "w");
            if (os == null) return "ERR:OPEN";
    ...
    @JavascriptInterface
    public String openPath(String relativePath) {
        try {
            final Uri uri = resolvePath(normalize(relativePath));
            if (uri == null) return "ERR:NOT_FOUND";
            activity.runOnUiThread(new Runnable() {
                @Override public void run() {
                    ...
                    try { activity.startActivity(i); }
                        ...
                        Intent chooser = Intent.createChooser(i, "Abrir reporte");
    ...
    @JavascriptInterface
    public String printPath(String relativePath, String jobName) {
        try {
            final Uri uri = resolvePath(normalize(relativePath));
            if (uri == null) return "ERR:NOT_FOUND";
            ...
            activity.runOnUiThread(new Runnable() {
                @Override public void run() {
                    ...
                    pm.print(label, new UriPrintAdapter(activity, uri, label), new PrintAttributes.Builder().build());
                }
            });
            return "OK";
        } catch (Throwable t) { return "ERR:" + safe(t); }
    ...
    public String rootDescription() {
        Uri t = getTreeUri();
        return t == null ? "Sin autorización" : "Carpeta autorizada · " + t.toString();
    }

    private Uri getTreeUri() {
        String s = prefs.getString(KEY_TREE, "");
        if (s == null || s.isEmpty()) return null;
        try { return Uri.parse(s); } catch (Throwable t) { return null; }
    }

    private Uri appRoot(boolean create) throws Exception {
        Uri tree = getTreeUri();
        if (tree == null) return null;
        String selectedName = displayName(tree);
        if (APP_FOLDER.equalsIgnoreCase(selectedName)) return tree;
        Uri found = findChild(tree, APP_FOLDER);
        if (found != null) return found;
        if (!create) return null;
        return DocumentsContract.createDocument(activity.getContentResolver(), tree, DocumentsContract.Document.MIME_TYPE_DIR, APP_FOLDER);
    }

    private Uri ensureDir(Uri parent, String name) throws Exception {
        Uri u = findChild(parent, name);
        if (u != null) return u;
        return DocumentsContract.createDocument(activity.getContentResolver(), parent, DocumentsContract.Document.MIME_TYPE_DIR, cleanName(name));
    }
    ...
    private Uri createFilePath(String relativePath, String mime, boolean replace) throws Exception {
        ...
        String[] parts = normalize(relativePath).split("/");
        Uri dir = root;
        for (int i = 0; i < parts.length - 1; i++) {
            if (!parts[i].isEmpty()) dir = ensureDir(dir, parts[i]);
        }
        String fileName = parts.length == 0 ? "documento.pdf" : cleanName(parts[parts.length - 1]);
        Uri old = findChild(dir, fileName);
        if (old != null && replace) {
            try { DocumentsContract.deleteDocument(activity.getContentResolver(), old); } catch (Throwable ignored) {}
        } else if (old != null) return old;
        return DocumentsContract.createDocument(activity.getContentResolver(), dir, mime, fileName);
    }

    private Uri resolvePath(String relativePath) throws Exception {
        Uri root = appRoot(false);
        if (root == null) return null;
        String[] parts = normalize(relativePath).split("/");
        Uri cur = root;
        for (String p : parts) {
    ...
    private String displayName(Uri uri) {
        Cursor c = null;
        try {
            c = activity.getContentResolver().query(uri, new String[]{DocumentsContract.Document.COLUMN_DISPLAY_NAME}, null, null, null);
    ...
    private static String cleanName(String n) {
        if (n == null || n.trim().isEmpty()) return "SIN_NOMBRE";
        return n.replace('/', '_').replace('\\', '_').replace(':', '-').trim();
    }

    private void notifyReady(final boolean ok) {
        if (activity.getWebView() == null) return;
        activity.getWebView().post(new Runnable() {
            @Override public void run() {
                activity.getWebView().evaluateJavascript("window.INKA_NATIVE_STORAGE_READY&&window.INKA_NATIVE_STORAGE_READY(" + (ok ? "true" : "false") + ");", null);
            }
        });
    }
    ...
    private static class UriPrintAdapter extends PrintDocumentAdapter {
        ...
        @Override public void onLayout(PrintAttributes oldAttributes, PrintAttributes newAttributes, CancellationSignal cancellationSignal, LayoutResultCallback callback, Bundle extras) {
            ...
            callback.onLayoutFinished(new PrintDocumentInfo.Builder(name + ".pdf").setContentType(PrintDocumentInfo.CONTENT_TYPE_DOCUMENT).build(), true);
        }
        @Override public void onWrite(PageRange[] pages, ParcelFileDescriptor destination, CancellationSignal cancellationSignal, WriteResultCallback callback) {
            ...
            } catch (Throwable t) { callback.onWriteFailed(safe(t)); }
            finally { try { if (in != null) in.close(); } catch (Throwable ignored) {} try { if (out != null) out.close(); } catch (Throwable ignored) {} }
        }
    }
}
